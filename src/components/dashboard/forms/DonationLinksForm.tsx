        ...
        try {
          await supabase.functions.invoke('send-payment-emails', {
            body: {
              ...
              message: 'We noticed you tried to create a donation link. Our service is currently available in Zimbabwe. We\'ll notify you via email when it becomes available in your country.',
            },
          });
        } catch (emailError) {
            ...
            <div className="space-y-3">
              ...
              {formData.image_url && (
                <div className="rounded-lg overflow-hidden border border-border w-full h-48 flex items-center justify-center bg-muted/40">
                  <img src={formData.image_url} alt="Campaign" className="w-full h-full object-cover" />
                </div>
              )}
              <Input
                type="file"
                ...
                onChange={async (e) => {
                  ...
                  try {
                    ...
                    const path = `${user.id}/donation-links/${Date.now()}.${ext}`;
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <div className="text-white/80 text-xs">Donation • Preview</div>
                </div>
              ...
              <div className="grid gap-8 lg:grid-cols-2">
                ...
                <div className="space-y-6">
                  <div className="rounded-xl overflow-hidden bg-muted/40 border border-border min-h-[260px] flex items-center justify-center">
                    {formData.image_url ? (
                      <img src={formData.image_url} alt={formData.title || 'Preview'} className="w-full h-full object-cover" />
                    ) : (
                ...
                <div className="flex flex-col gap-6">
                  <div className="space-y-3">
                    <h3 className="text-2xl font-bold text-foreground">{formData.title || 'Campaign title'}</h3>
                    {formData.description && (
                      <p className="text-muted-foreground leading-relaxed">{formData.description}</p>
                    )}
                    <div className="text-2xl font-bold text-foreground">
                      {(formData.goal_amount && userCurrency) ? `${userCurrency} ${Number(formData.goal_amount).toFixed(2)} goal` : ''}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      >
                        Donate {formData.goal_amount && userCurrency ? `${userCurrency} ${Number(formData.goal_amount).toFixed(2)}` : ''}
                      </Button>
                    </div>
                  </div>
                </div>
