...
var handler = async (event) => {
  ...
  try {
    ...
    const { email, password, profile = {}, merchant = {} } = body;
    ...
    if (createErr || !created.user) return json(400, { error: "Sign up failed", details: createErr?.message });
    ...
    if (upErr) return json(500, { error: "Failed to write profile", details: upErr.message });
    ...
    if (!merchantId) {
      const businessName = merchant.name || email.split("@")[0];
      const { data: mIns, error: mErr } = await supabase.from("merchants").insert({
        ...
        contact_email: merchant.contact_email || email
      }).select("id").single();
      if (mErr) return json(500, { error: "Failed to create merchant", details: mErr.message });
      merchantId = mIns.id;
