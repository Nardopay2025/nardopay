...
// netlify/functions/payment-links.ts
var payment_links_exports = {};
__export(payment_links_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(payment_links_exports);
...
var handler = async (event) => {
  ...
  try {
    ...
    const { data: mRow, error: mErr } = await supabase.from("merchant_members").select("merchant_id").eq("auth_user_id", userId).limit(1).maybeSingle();
    if (mErr || !mRow) return json(403, { error: "No merchant membership found" });
    const merchantId = mRow.merchant_id;
    const body = event.body ? JSON.parse(event.body) : {};
    const { type = "one_time", title, amount, currency = "USD", description = "", thankYouMessage = "Thank you!", redirectUrl = "", goalAmount, goalCurrency, billingCycle, trialDays } = body;
    if (!title || amount === void 0) return json(400, { error: "Missing title or amount" });
    ...
    if (insErr) return json(500, { error: "Failed to create link", details: insErr.message });
    if (type === "donation" && goalAmount) {
      await supabase.from("donation_links").insert({
        link_id: slug,
        goal_amount: Number(goalAmount),
        goal_currency: goalCurrency || currency
      });
    }
    ...
    return json(200, { link: { id: slug, link: linkPath } });
  } catch (err) {
