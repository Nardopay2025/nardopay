...
// netlify/functions/auth-bootstrap.ts
var auth_bootstrap_exports = {};
__export(auth_bootstrap_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(auth_bootstrap_exports);
...
var handler = async (event) => {
  ...
  try {
    ...
    const { data: existing } = await supabase.from("merchants").select("id").eq("owner_auth_user_id", user.id).limit(1).maybeSingle();
    let merchantId = existing?.id;
    if (!merchantId) {
      const businessName = merchantPayload.name || user.user_metadata?.company || user.email?.split("@")[0] || "Merchant";
      const toInsert = {
        owner_auth_user_id: user.id,
        name: businessName,
        status: "active",
        country_code: merchantPayload.country_code || null,
        default_currency: merchantPayload.default_currency || null,
        website: merchantPayload.website || null,
        contact_email: merchantPayload.contact_email || user.email || null
      };
      const { data: mIns, error: crtErr } = await supabase.from("merchants").insert(toInsert).select("id").single();
      if (crtErr) return json(500, { error: "Failed to create merchant", details: crtErr.message });
      merchantId = mIns.id;
    ...
    return json(200, { merchant_id: merchantId });
  } catch (err) {
